...
void RenderText::drawText(int alignment, std::string text, GLfloat x, GLfloat y, GLfloat scale, glm::vec3 color)
{
    ...
    //prog->bind();
    glUniform3f(prog->getUniform("textColor"), color.x, color.y, color.z);
...
void RenderText::drawText(int alignment, std::string text, GLfloat x, GLfloat y, GLfloat xscale, GLfloat yscale, glm::vec3 color)
{
    // Activate corresponding render state	
    //prog->bind();
    glUniform3f(prog->getUniform("textColor"), color.x, color.y, color.z);
    ...
    {
        for (c = text.begin(); c != text.end(); c++)
            x_len += (Characters[*c].Advance >> 6) * xscale;
    }
    ...
    for (c = text.begin(); c != text.end(); c++)
    {
        ...
        GLfloat xpos = x + ch.Bearing.x * xscale - x_len / 2.f;
        ...
        glDrawArrays(GL_TRIANGLES, 0, 6);
        // Now advance cursors for next glyph (note that advance is number of 1/64 pixels)
        x += (ch.Advance >> 6) * xscale; // Bitshift by 6 to get value in pixels (2^6 = 64 (divide amount of 1/64th pixels by 64 to get amount of pixels))
    }
    glBindVertexArray(0);
    glBindTexture(GL_TEXTURE_2D, 0);
    //prog->unbind();
